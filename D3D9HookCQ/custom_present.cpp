...
{
private:
	unsigned t1, t2, fcount,cq_cursor;
	...
	UINT font_weight,period_frames,cq_msg_show_frames;
	RECT rText, rTextShadow;
	int formatFlag;
	D3DCOLOR color_text, color_shadow;
	D3DVIEWPORT9 viewport;
	std::wstring cq_msg;
	int cq_frame_count,cq_max_msg_length;
public:
	D3DXCustomPresent():t1(0),t2(0),fcount(0),formatFlag(0),cq_cursor(0),cq_frame_count(0)
	{
	}
	D3DXCustomPresent(D3DXCustomPresent &&other)
	{
		...
		cq_cursor = std::move(other.cq_cursor);
		cq_frame_count = std::move(other.cq_frame_count);
	}
	~D3DXCustomPresent()
	{
		Uninit();
...
#define F(_i_str) (float)_wtof(_i_str)
		...
		GetInitConfInt(cq_msg_show_frames, 300);
		GetInitConfInt(cq_max_msg_length, 64);
	...
	void Draw()
	{
		...
		if (CQGetSharedMemory().current_pos != cq_cursor)
		{
			CQSharedMemory& cq = CQGetSharedMemory();
			CQRecord& r = cq.records[cq_cursor];
			cq_cursor = cq.current_pos;
			cq_frame_count = cq_msg_show_frames;
			std::string amsg = r.user_name;
			if (strlen(r.remark_name))
				amsg = std::string(r.remark_name) + "(" + amsg + ")";
			if (strlen(r.group_name))
				amsg += ":" + std::string(r.group_name);
			amsg += ":" + std::string(r.message);
			TCHAR wmsg[800];
			if (amsg.size() > ARRAYSIZE(wmsg) - 1)
				amsg.resize(ARRAYSIZE(wmsg) - 1);
			int wch_count_need=MultiByteToWideChar(CP_UTF8, NULL, amsg.c_str(), amsg.length(), 0, 0);
			if (wch_count_need + 1 > ARRAYSIZE(wmsg))
				lstrcpy(wmsg, TEXT("传送的数据太多，无法显示。"));
			else
			{
				MultiByteToWideChar(CP_UTF8, NULL, amsg.c_str(), amsg.length(), wmsg, wch_count_need);
				wmsg[wch_count_need] = 0;
			}
			cq_msg = wmsg;
			if (cq_msg.length() > cq_max_msg_length)
			{
				cq_msg.resize(cq_max_msg_length);
				cq_msg += TEXT("...");
			}
		}
		if (cq_frame_count)
		{
			pFont->DrawText(NULL, cq_msg.c_str(), (int)cq_msg.length(), &rTextShadow, formatFlag, color_shadow);
			pFont->DrawText(NULL, cq_msg.c_str(), (int)cq_msg.length(), &rText, formatFlag, color_text);
			cq_frame_count--;
		}
		else
		{
